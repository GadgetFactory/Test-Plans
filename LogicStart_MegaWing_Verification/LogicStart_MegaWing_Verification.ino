...
static void joyTest()
{
	REGISTER( SEVENSEGBASE, 7 ) = 0;
	REGISTER( SEVENSEGBASE, 6 ) = SEGF|SEGB|SEGG|SEGC|SEGD;
	REGISTER( SEVENSEGBASE, 5 ) = SEGA|SEGB|SEGC|SEGD|SEGE|SEGF;
	REGISTER( SEVENSEGBASE, 4 ) = SEGB|SEGC|SEGD|SEGE;
}

static void adcTest()
{
	REGISTER( SEVENSEGBASE, 7 ) = 0;
	REGISTER( SEVENSEGBASE, 6 ) = SEGD|SEGE|SEGG;
	REGISTER( SEVENSEGBASE, 5 ) = SEGB|SEGC|SEGD|SEGE|SEGG;
	REGISTER( SEVENSEGBASE, 4 ) = SEGA|SEGB|SEGC|SEGE|SEGF|SEGG;
}

static void ledTest()
{
	REGISTER( SEVENSEGBASE, 7 ) = 0;
	REGISTER( SEVENSEGBASE, 6 ) = SEGA|SEGB|SEGC|SEGD;
	REGISTER( SEVENSEGBASE, 5 ) = SEGA|SEGD|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 4 ) = SEGD|SEGE|SEGF;
}
...
static void up()
{
	REGISTER( SEVENSEGBASE, 7 ) = 0;
	REGISTER( SEVENSEGBASE, 6 ) = 0;
	REGISTER( SEVENSEGBASE, 5 ) = SEGA|SEGB|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 4 ) = SEGB|SEGC|SEGD|SEGE|SEGF;
}

static void down()
{
	REGISTER( SEVENSEGBASE, 7 ) = SEGA|SEGB|SEGC|SEGE|SEGF;
	REGISTER( SEVENSEGBASE, 6 ) = SEGB|SEGC|SEGD|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 5 ) = SEGA|SEGB|SEGC|SEGD|SEGE|SEGF;
	REGISTER( SEVENSEGBASE, 4 ) = SEGA|SEGB|SEGC|SEGD;
}

static void left()
{
	REGISTER( SEVENSEGBASE, 7 ) = SEGD|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 6 ) = SEGA|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 5 ) = SEGA|SEGD|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 4 ) = SEGD|SEGE|SEGF;
}
...
{
	REGISTER( SEVENSEGBASE, 7 ) = SEGD|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 6 ) = SEGB|SEGC|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 5 ) = SEGA|SEGC|SEGD|SEGE|SEGF;
	REGISTER( SEVENSEGBASE, 4 ) = SEGA|SEGB|SEGC|SEGE|SEGG;
}

static void sel()
{
	REGISTER( SEVENSEGBASE, 7 ) = 0;
	REGISTER( SEVENSEGBASE, 6 ) = SEGD|SEGE|SEGF;
	REGISTER( SEVENSEGBASE, 5 ) = SEGA|SEGD|SEGE|SEGF|SEGG;
	REGISTER( SEVENSEGBASE, 4 ) = SEGA|SEGC|SEGD|SEGF;
}
...
void loop()
{
	...
	if ((extcnt & 0x17) == 0) {
		...
		//REGISTER( SEVENSEGBASE, 0 ) = 0xF | (lpwm<<16);
          ...
          //LED Test Mode
          if (digitalRead(LS_SWITCH1)){
            if (timeout > 0){
              ledTest();    
            } 
            else{           
              set_sevenseg_value(8888,0);              
            }
          }  
  
          //LED Test Joystick         
          if (digitalRead(LS_SWITCH2)){
            if (timeout > 0){
              joyTest();    
            }
	    if (!digitalRead(LS_JOY_SELECT))
                sel();   
	    if (!digitalRead(LS_JOY_DOWN))
              down(); 
	    if (!digitalRead(LS_JOY_RIGHT))
              right();
